...
export default {
  ...
  data() {
    return {
      payInfo: {},//支付信息，里面含有codeUrl，用于生成二维码
      timer:null,
      code:'',//保存用户支付状态
    };
  },
  ...
  methods: {
    async getPayInfo() {
      let result = await this.$API.reqOrderPay(this.$route.query.orderId);//携带订单号发送请求获取支付信息
      if (result.code == 200) {
    ...
    //点击立即支付
    async pay() {
      //qrcdoe.toDataUrl需要传递一个字符串，该函数返回Promise对象，值为二维码图片的链接
      let imgUrl = await qrcode.toDataURL(this.payInfo.codeUrl);
      //this.$alert()包含三个参数，具体可查阅文档。第一个参数为message消息正文内容，第二个参数为title，第三个参数为配置对象
      this.$alert(`<img src=${imgUrl}>`,'请微信支付', {
        dangerouslyUseHTMLString: true,//将第一个参数message作为html片段处理
        center: true,
        showClose: true,
