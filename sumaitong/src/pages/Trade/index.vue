...
export default {
  ...
  methods: {
    async getData() {
      //获取用户地址 需要用户已经登陆，此处用13700000000 111111测试
      let result = await this.$API.reqAddressInfo();//{code: 200, message: '成功', data: Array(4), ok: true}
      if (result.code == 200) {
    ...
    async submitOrder() {
      ...
      let data = {
        consignee: this.defaultAddress.consignee,//收件人姓名
        consigneeTel: this.defaultAddress.phoneNum,//电话
        deliveryAddress: this.defaultAddress.fullAddress,//地址
        paymentWay: "ONLINE",//在线支付
        orderComment: this.message,//用户留言信息
        orderDetailList: this.orderInfo.detailArrayList,//购物车清单
      };
      let result = await this.$API.reqSubmitOrder(tradeNo, data);//返回数据为订单号
       if(result.code==200){
          this.orderId = result.data;
          this.$router.push('/pay/?orderId='+this.orderId);//路由跳转+传参
       }else{
