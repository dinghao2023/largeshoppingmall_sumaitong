          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(saleAttr, index) in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd changepirce="0" :class="{ active: saleAttrValue.isChecked == 1 }" v-for="(saleAttrValue, index) in saleAttr.spuSaleAttrValueList" :key="saleAttrValue.id" 
...
export default {
  ...
  computed: {
    ...
    imageList() {
      return this.skuInfo.skuImageList || [];//传递给Zoom组件的数据，如果skuInfo是一个空对象，skuInfo.skuImageList则是undefined，所以这里要加上||[]
    },
  },
  methods: {
    changeChecked(saleAttrValue, list) {
      //把全部的属性值isChecked属性变为0，点击的属性值为1
      list.forEach((item) => {
        item.isChecked = "0";
      });
    ...
    changeCount(event) {
      ...
      if (isNaN(result) || result < 1) {//考虑非法的情况
        this.count = 1;
      } else {//如果result出现了小数取整
    ...
    async addShopCart() {
      try {
        ...
        sessionStorage.setItem('SKUINFO',JSON.stringify(this.skuInfo));//通过会话存储，需要先转为字符串。
        this.$router.push({name:'addcartsuccess',query:{skuNum:this.count}});//路由跳转时需要将产品信息传递过去。将产品个数可以通过query传过去，但产品信息不能通过query传过去，因为query只能传递字符串，所以将产品信息存储在sessionStorage中，在跳转页面的时候将sessionStorage中的数据取出
      } catch (error) {
        alert("加入购物车失败");
      }
    },
