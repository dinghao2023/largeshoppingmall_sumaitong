  ...
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <!-- is:紧随的判断是否相等规则 -->
         <input type="text" placeholder="请再次输入密码确认" v-model="password1" name="password1" v-validate="{ required: true, is:password}" :class="{ invalid: errors.has('password1') }">
      ...
      async getCode(){
        ...
        if(phone){
          try {
            ...
            this.code = this.$store.state.user.code;//组件中的code值设置为仓库中获取的code值
          } catch (error) {
            throw new Error(error);
          }
        }
      },
      async register(){//点击完成注册按钮
      const success = await this.$validator.validateAll();//vee-valadiate提供的一个方法，如果表单验证全部成功，则返回true
      if(success){
        const {phone,code,password,password1} = this;
          try {
            //  let result = await this.$store.dispatch('userRegister',{phone,code,password});
            //  console.log(result);//ok  用户名已被注册
